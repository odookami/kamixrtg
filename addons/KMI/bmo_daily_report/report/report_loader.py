...
class LoaderXlsx(models.AbstractModel):
	_name = 'report.bmo_daily_report.report_loader'
	_description = 'Daily Report Loader'
	...
	def write_dictionary(self, x, n_row, worksheet, workbook, reset=False):
		cell_dict = {}
		if x._name == 'kmi.loader.general.checks':
			cell_dict = {
		...
		if x._name == 'kmi.loader.params.value':
	...
	def generate_xlsx_report(self, workbook, data, partners):
		...
		for obj in partners:
			...
			worksheet.merge_range('E4:R5', 'Form Laporan Harian Loader (OBOL)', self.formating_styles(workbook).get('company_format'))
...
# =========================================================================================        
			...
			worksheet.merge_range('R14:U15', '', self.formating_styles(workbook).get('company_format'))
			...
			worksheet.merge_range('T9:U9', str(obj.date.strftime("%d-%m-%Y")), self.formating_styles(workbook).get('table_center'))
			...
			worksheet.merge_range('A11:C11', 'Total Reject', self.formating_styles(workbook).get('table_left'))
			...
			worksheet.merge_range('D11:E11', obj.total_reject_produk, self.formating_styles(workbook).get('table_center'))
			...
			worksheet.merge_range('G9:I9', 'Product Name ', self.formating_styles(workbook).get('table_left'))
			worksheet.merge_range('G10:I10', 'Start Loading Actual', self.formating_styles(workbook).get('table_left'))
			worksheet.merge_range('G11:I11', 'Finish Loading Actual ', self.formating_styles(workbook).get('table_left'))
			worksheet.merge_range('G12:I12', 'Reject  ', self.formating_styles(workbook).get('table_left'))
			c = 9
			...
			for i in general_line:
				...
				worksheet.write(8, c, i.product_id.display_name, self.formating_styles(workbook).get('table_left'))
				worksheet.write(9, c, sa, self.formating_styles(workbook).get('table_center'))
				worksheet.write(10, c, fa, self.formating_styles(workbook).get('table_center'))
				worksheet.write(11, c, i.reject_batch, self.formating_styles(workbook).get('table_center'))
				c += 1
			worksheet.merge_range('A13:Q13', 'Notes', self.formating_styles(workbook).get('table_center'))
			worksheet.merge_range('A14:Q15', obj.note or '', self.formating_styles(workbook).get('company_format'))
# =========================================================================================       

			# PERSIAPAN AWAL PENCETAKAN GENERAL CHECKS
			row = 16
			worksheet.merge_range('A{row}:U{row}'.format(row=row), 'GENERAL CHECKS ( early shift checks)', self.formating_styles(workbook).get('company_format'))
			row += 1
			worksheet.merge_range('A{row}:C{row}'.format(row=row), 'Parameter', self.formating_styles(workbook).get('table_row_center'))
			worksheet.merge_range('D{row}:E{row}'.format(row=row), 'Std', self.formating_styles(workbook).get('table_row_center'))
			worksheet.merge_range('F{row}:G{row}'.format(row=row), 'Actual', self.formating_styles(workbook).get('table_row_center'))
			worksheet.merge_range('H{row}:J{row}'.format(row=row), 'Parameter', self.formating_styles(workbook).get('table_row_center'))
			worksheet.merge_range('K{row}:L{row}'.format(row=row), 'Std', self.formating_styles(workbook).get('table_row_center'))
			worksheet.merge_range('M{row}:N{row}'.format(row=row), 'Actual', self.formating_styles(workbook).get('table_row_center'))
			worksheet.merge_range('O{row}:P{row}'.format(row=row), 'Parameter', self.formating_styles(workbook).get('table_row_center'))
			worksheet.write('Q{row}'.format(row=row), 'Std', self.formating_styles(workbook).get('table_row_center'))
			worksheet.write('R{row}'.format(row=row), 'Actual', self.formating_styles(workbook).get('table_row_center'))
			worksheet.merge_range('S{row}:U{row}'.format(row=row), 'Notes :', self.formating_styles(workbook).get('table_row_center'))
			row += 1

			f_row = row # First Row untuk sebagai acuan row ketika reset general checks
			n_row = f_row # row yang digunakan untuk looping pertama general checks
			lrow = 1
			reset = False
			...
			n_row = f_row
			lrow = n_row + 5
			worksheet.merge_range('S{frow}:U{lrow}'.format(frow=n_row,lrow=lrow), obj.note_checks or '', self.formating_styles(workbook).get('table_row_center'))
			n_row = lrow + 1
