...
class sale_day_book_wizard(models.TransientModel):
    ...
    category = fields.Many2many('product.category', 'categ_wiz_rel', 'categ', 'wiz' , domain = [('property_valuation','=','real_time')])
    location_id = fields.Many2one('stock.location', string= 'Location')
    company_id = fields.Many2one('res.company', string= 'Company', default=lambda self: self.env['res.company']._company_default_get('account.move'))
    ...
    product_ids = fields.Many2many('product.product', 'rel_product_val_wizard', string="Product", domain = [('type','=','product')])
    ...
    def get_lines(self, data):
            ...
            for product in  product_res :
                ...
                for move in stock_move_line :
                    ...
                    if move.picking_id.picking_type_id.code == "incoming" :
                        if data['location_id'] :
                            ...
                            incoming = incoming + move.product_uom_qty
                # =============================OLD==============================
                stock_val_layer_old = self.env['stock.valuation.layer'].search([
                        ('product_id','=',product.id),
                        ('create_date',"<=",data['start_date']),                        
                        ] )
                cost_old = 0
                count_old = 0
                for layer in stock_val_layer_old :
                        if layer.stock_move_id.picking_id.picking_type_id.code == "incoming" :
                            cost_old = cost_old + layer.unit_cost
                ...
                avg_cost_old = 0
                if count_old > 0 :
                    avg_cost_old = cost_old / count_old

                if not stock_val_layer_old and  avg_cost_old == 0:
                    avg_cost_old = product.standard_price
                # =============================RUN+++++++++++++++++++++++++++++
                ...
                method = ''
                price_used = price_used_old = product.standard_price
                if product.categ_id.property_cost_method == 'average' :
                    method = 'Average Cost (AVCO)'
                    price_used = avg_cost 
                    price_used_old = avg_cost_old 

                elif product.categ_id.property_cost_method == 'standard' :
                    method = 'Standard Price'
                    price_used = product.standard_price
                    price_used_old = product.standard_price
                opening_data = opening[product.id]['qty_available'] or 0
                opening_price = price_used_old
                vals = {
                        'sku': product.default_code or '',
                        'name': product.name or '',
                        'category': product.categ_id.name or '' ,
                        'costing_method' : method,
                        'cost_price_old': price_used_old or 0,
                        'cost_price': price_used or 0,
                        'available':  0 ,
                        'virtual':   0,
                        'beginning': opening[product.id]['qty_available'] or 0,
                        'total_value_old': opening_data * opening_price,
    ...
    def get_data(self,data):
        ...
        for product in  product_res :
            ...
            # ('force_date','>=',data['start_date']),
            # ('force_date',"<=",data['end_date']),
            cost = 0
            count = 0
            for layer in stock_val_layer :
                    if layer.stock_move_id.picking_id.picking_type_id.code == "incoming" :
    ...
    def print_exl_report(self):
        if xlwt:
            ...
            if self.display_sum:
                ...
                worksheet.write(8, 4, 'Cost Price Old', style_table_header)
                worksheet.write(8, 5, 'Cost Price', style_table_header)
                worksheet.write(8, 6, 'Beginning', style_table_header)
                worksheet.write(8, 7, 'Beginning Valuation', style_table_header)
                worksheet.write(8, 8, 'Internal', style_table_header)
                worksheet.write(8, 9, 'Received', style_table_header)
                worksheet.write(8, 10, 'Sales', style_table_header)
                worksheet.write(8, 11, 'Adjustment', style_table_header)
                worksheet.write(8, 12, 'Ending', style_table_header)
                worksheet.write(8, 13, 'Valuation', style_table_header)
                prod_row = 9
                prod_col = 0
                ...
                for each in get_line:
                    ...
                    worksheet.write(prod_row, prod_col+4, each['cost_price_old'], style)
                    worksheet.write(prod_row, prod_col+5, each['cost_price'], style)
                    worksheet.write(prod_row, prod_col+6, each['beginning'], style)
                    worksheet.write(prod_row, prod_col+7, each['total_value_old'], style)
                    ...
                    worksheet.write(prod_row, prod_col+13, each['total_value'], style)
                    prod_row = prod_row + 1
